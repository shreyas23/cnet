...
class CNet(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.training or (not self._args['finetuning'] and not self._args['evaluation']):
            ...
            output_dict_r = self.run_pwcFigure 3: Training cycle of Competitive Collaboration: ThemoderatorMdrives two competitors{R,F}(first phase,left). Later, the competitors collaborate to train the moder-ator to ensure fair competition in the next iteration (secondphase, right).motion segmentation networkMselects networks(D,C)on pixels that are static and selectsFon pixels that belongto moving regions. The competition ensures that(D,C)rea-sons only about the static parts and prevents moving pixelsfrom corrupting its training. Similarly, it prevents any staticpixels from appearing in the training loss ofF, thereby im-proving its performance in the moving regions. In the secondphase of the training cycle, the competitors(D,C)andFnow collaborate to reason about static scene and movingregions by forming a consensus that is used as a loss fortraining the moderator,M.  In the rest of this section, weformulate the joint unsupervised estimation of depth, cameramotion, optical flow and motion segmentation within thisframework.Notation.We use{Dθ,Cφ,Fψ,Mχ}, to denote the net-works  that  estimate  depth,  camera  motion,  optical  flowand  motion  segmentation  respectively.The  subscripts{θ,φ,ψ,χ}are the network parameters.  We will omit thesubscripts in several places for brevity.   Consider an im-age sequenceI−,I,I+with target frameIand temporallyneighboring reference framesI−,I+.  In general, we canhave many neighboring frames. In our implementation, weuse 5-frame sequences forCφandMχbut for simplicity use3 frames to describe our approach. We estimate the depth ofthe target frame asd=Dθ(I).(3)We estimate the camera motion,e, of each of the referenceframesI−,(
