...
def pixel2cam(depth, intrinsics_inv, cam_tform=None):
    global pixel_coords
...
def flow_warp(img, flow, padding_mode='zeros'):
    ...
    grid_x = torch.arange(0, w, requires_grad=False).view(1, 1, w).expand(1,h,w).type_as(u).expand_as(u)  # [bs, H, W]
    grid_y = torch.arange(0, h, requires_grad=False).view(1, h, 1).expand(1,h,w).type_as(v).expand_as(v)  # [bs, H, W]
...
def pose2flow(depth, pose, intrinsics, intrinsics_inv, rotation_mode='euler', cam_tform=None, padding_mode=None):
    """
    ...
    if cam_tform is not None:
      check_sizes(cam_tform, 'cam2cam transform', 'B34')
