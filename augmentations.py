...
class PhotometricAugmentation(nn.Module):
    ...
    def forward(self, *args):
        ...
        rand_shift = torch.cat(
            [rand_shift for i in range(0, num_images)], dim=1)
...
class Augmentation_ScaleCrop(nn.Module):
    ...
    def find_invalid(self, img_size, params):
        ...
        pt_mat = torch.cat([torch.cat([hf_w, hf_o, hf_o], dim=2), torch.cat(
            [hf_o, hf_h, hf_o], dim=2), torch.cat([hf_o, hf_o, hf_i], dim=2)], dim=1)
        ...
        invalid = (((x_tf <= -img_hf_w) | (y_tf <= -img_hf_h) | (x_tf >= img_hf_w)
                    | (y_tf >= img_hf_h)).sum(dim=1, keepdim=True) > 0).float()
    ...
    def calculate_tform_and_grids(self, img_size, resize, params):
        ...
        grid_img_ww = pts_tform[:, 0, :, :] / float(img_size[1]) * 2
